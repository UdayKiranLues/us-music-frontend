...
const SongList = memo(({ songs, showIndex = false, onSongDeleted }) => {
...
const SongListItem = memo(({ song, index, isCurrentSong, isPlaying, showIndex, onPlay, user, onDelete }) => {
  ...
  const handleDelete = async (e) => {
    ...
    try {
      await axios.delete(`/api/v1/songs/${song._id || song.id}`);
      showToast('Song deleted successfully', '✅');
      if (onDelete) onDelete(song._id || song.id);
    } catch (err) {
    ...
    >
      ...
      <div className="flex items-center gap-4">
        ...
        */}
        ...
        {(user?.role === 'admin' || (user?._id === (song.uploadedBy || song.createdBy))) && (
          <motion.button
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
            className="opacity-0 group-hover:opacity-100 transition-opacity p-2 hover:bg-red-500/20 rounded-full text-red-400 hover:text-red-300"
